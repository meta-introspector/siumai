...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(api_key) = std::env::var("OPENAI_API_KEY") {
        ...
        match demonstrate_openai_embeddings(&api_key, &texts).await {
            ...
            Err(e) => println!("❌ OpenAI embeddings failed: {}\n", e),
        }
    } else {
    ...
    match demonstrate_ollama_embeddings(&texts).await {
        ...
        Err(e) => println!("❌ Ollama embeddings failed: {}\n", e),
    }
...
async fn demonstrate_openai_embeddings(
    ...
    if embedding_proxy.is_reported_as_supported() {
        ...
        println!("      Max tokens per request: {}", embedding_proxy.max_tokens_per_embedding());
        println!("      Supported models: {:?}", embedding_proxy.supported_embedding_models());
        
        let response2 = embedding_proxy.embed(vec![texts[0].clone()]).await?;
        println!("      Single embedding dimension: {}", response2.embeddings[0].len());
    } else {
...
async fn demonstrate_ollama_embeddings(
    texts: &[String],
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let embedding_proxy = client.embedding_capability();
    println!("      Capability supported: {}", embedding_proxy.is_reported_as_supported());
    println!("      Default dimension: {}", embedding_proxy.embedding_dimension());
...
async fn demonstrate_capability_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(api_key) = std::env::var("ANTHROPIC_API_KEY") {
        ...
        println!("      Reported as supported: {}", embedding_proxy.is_reported_as_supported());
...
async fn demonstrate_interface_comparison(
    ...
    println!("      Unified: {} embeddings, {} dimensions", 
    ...
    let specific_client = Provider::openai()
        .api_key(api_key)
        .build()
        .await?;
    
    let specific_response = specific_client.embed(texts.to_vec()).await?;
    println!("      Provider-specific: {} embeddings, {} dimensions", 
