...
实际回答：UTF-8编码确实需要特殊处理。"#;
    ...
    assert!(
        ...
        "Should contain opening thinking tag"
    );
    assert!(
        result.contains("</think>"),
        "Should contain closing thinking tag"
    );

    // Verify Chinese content
    assert!(
        result.contains("这是一个复杂的问题"),
        "Should contain Chinese thinking content"
    );
    assert!(
        result.contains("UTF-8编码"),
        "Should contain UTF-8 reference"
    );
...
fn test_utf8_decoder_performance_with_large_content() {
    ...
    assert!(
        !result.contains('�'),
        "Large content processing should not produce corruption"
    );
